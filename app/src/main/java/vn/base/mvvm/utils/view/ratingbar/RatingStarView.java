...
public class RatingStarView extends View implements View.OnClickListener {
    ...
    private boolean unRateable = false;
    ...
    private OnRatingChanged mOnStarChangeListener;
    private OnRatingConfirm mOnStarConfirmListener;
    ...
    private void loadAttributes(AttributeSet attrs, int defStyle) {
        ...
        unRateable = a.getBoolean(R.styleable.RatingStarView_rsv_un_rate, false);
    ...
    public void setRating(float rating) {
        if (rating != this.rating) {
            ...
            if (mOnStarChangeListener!=null)
                mOnStarChangeListener.onStarChanged(rating);
        }
    }
...
//        Log.d(TAG, "drawPartialStar percent = " + percent);
    ...
    public void setOnStarChangeListener(OnRatingChanged l){
        mOnStarChangeListener = l;
    }

    public void setOnStarConfirmListener(OnRatingConfirm l){
        mOnStarConfirmListener = l;
    }
    ...
    private void changeRatingByClick() {
        ...
        for (int i = 1; i <= starCount; i++) {
            ...
            if (clickedX >= left && clickedX <= right) {
                if (this.rating == i) {
                    if (unRateable) {
                        if (mOnStarConfirmListener!=null)
                            mOnStarConfirmListener.onStarSelected(i);
                        else
                            setRating(0);
                    }
                } else {
